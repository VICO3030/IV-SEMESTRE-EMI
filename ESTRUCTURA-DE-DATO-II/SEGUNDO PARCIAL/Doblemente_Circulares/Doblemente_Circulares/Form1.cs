...
namespace Doblemente_Circulares
{
    public partial class Frm_Principal : Form
    {
        ...
        {
            if ((l1.EleminarActul(int.Parse(txt_eleminar.Text))) == false)
            {
                MessageBox.Show("EL numero no existe");
            }
            else
            {
                if ((l1.EleminarActul(int.Parse(txt_eleminar.Text))) == true)
                {
        ...
        {
            l1.Insertar(int.Parse(txt_insertar.Text));
            MessageBox.Show("Se inserto el numero!!");
            LST_NUMEROS.Items.Clear();
            txt_insertar.Clear();
            Mostrar();
        }
